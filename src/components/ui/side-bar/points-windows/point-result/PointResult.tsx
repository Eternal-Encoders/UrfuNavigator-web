...
import styles from './point-result.module.css'
import { floorSet } from '../../../../../features/floor/floorSlice';

interface PointResultProps {
    data: IGraphPoint,
    setName: (name: string) => void,
    setType: (type: PointTypes | undefined) => void,
    isEnd: boolean
}
...
function PointResult({data, setName, setType, isEnd}: PointResultProps) {
    ...
    return (
        <>
            <li>
                <button className={styles['search-result-btn']} onClick={ clickHandler }>
                    <p className={styles['search-result-btn-name']}>{ data.names.join(', ') }</p>
                    <div className={styles['search-result-addition']}>
                        <div className={styles['search-result-addition-img']} style={{ backgroundColor: InstColors.get(data.institute) }}/>
